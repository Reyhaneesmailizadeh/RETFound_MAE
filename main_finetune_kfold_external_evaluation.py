...
from util.datasets import build_dataset_usa
...
def get_args_parser():
    ...
    parser.add_argument('--usa_dir', default='/content/drive/MyDrive/finetune_data_GCIPL_USA',
                        help='USA directory for external evaluation')
    return parser
...
def main(args):
    ...
    dataset_test_usa = build_dataset_usa(is_train='test', args=args)
    sampler_test_usa = torch.utils.data.SequentialSampler(dataset_test_usa)
    data_loader_test_usa = torch.utils.data.DataLoader(
            dataset_test_usa, sampler=sampler_test_usa,
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset_full)):
        ...
        state_dict_best = torch.load(args.task+f'checkpoint-best-{fold}.pth', map_location='cpu')
        ...
        #start evaluating the model on USA dataset(all 32 data treated as test)
        print('Evaluating on external (USA) dataset')
        test_stats_usa, auc_roc_usa, f1_usa, sensitivity_usa, specificity_usa, precision_usa = evaluate(data_loader_test_usa, model_without_ddp, device,args.task,epoch=0, mode='test',num_class=args.nb_classes)
