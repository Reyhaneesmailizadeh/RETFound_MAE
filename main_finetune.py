...
#         criterion = SoftTargetCrossEntropy(weight = class_weights)
...
#             if log_writer is not None:
#                 log_writer.flush()
#             with open(os.path.join(args.output_dir, "finetuning-log.txt"), mode="a", encoding="utf-8") as f:
...
def main(args):
    ...
    # Load the entire dataset (combined train and validation datasets)
    full_dataset = build_dataset(is_train='train', args=args)  # Adjusted to load combined data

    # Initialize the test dataset which remains constant across folds
    dataset_test = build_dataset(is_train='test', args=args)
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(full_dataset)):
        ...
        sampler_train = torch.utils.data.RandomSampler(train_subset) if not args.distributed else torch.utils.data.DistributedSampler(
            train_subset, num_replicas=misc.get_world_size(), rank=misc.get_rank(), shuffle=True
        )
        sampler_val = torch.utils.data.SequentialSampler(val_subset)

        data_loader_train = torch.utils.data.DataLoader(
            train_subset, sampler=sampler_train,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=args.pin_mem,
            drop_last=True,
        ...
        data_loader_val = torch.utils.data.DataLoader(
            val_subset, sampler=sampler_val,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=args.pin_mem,
            drop_last=False
        ...
        # Load any pretrained model if specified
        if args.finetune and not args.eval:
            checkpoint = torch.load(args.finetune, map_location='cpu')
            print("Loading pre-trained checkpoint from: %s" % args.finetune)
        ...
        criterion = LabelSmoothingCrossEntropy(smoothing=args.smoothing) if args.smoothing > 0. else torch.nn.CrossEntropyLoss()
        
        if args.eval:
            evaluate(data_loader_test, model, device, args.task, epoch=0, mode='test', num_class=args.nb_classes)
            continue
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            train_stats = train_one_epoch(
                model, criterion, data_loader_train,
                optimizer, device, epoch, loss_scaler,
                args.clip_grad, None,  # Assuming no mixup
                log_writer=None,
                args=args
            )

            val_stats, _, _, _, _, _ = evaluate(data_loader_val, model, device, args.task, epoch, mode='val', num_class=args.nb_classes)
        ...
        # Final evaluation on test set after training each fold
        evaluate(data_loader_test, model, device, args.task, epoch=0, mode='test', num_class=args.nb_classes)
